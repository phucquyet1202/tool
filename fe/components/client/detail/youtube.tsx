...
import {
  StartYouTubeLivestream,
  StopYouTubeLivestream,
} from "@/api/livestream";
...
export default function LiveDetailYoutubePage() {
  const router = useRouter();
  const queryClient = useQueryClient();

  // State quản lý user
  const [statusYt, setStatusYt] = useState(false);

  // Fetch user info để lấy statusYt
  type UserResponse = {
    data?: {
      status_yt?: boolean;
  ...
  useEffect(() => {
    if (userData?.data) {
      setStatusYt(userData?.data?.status_yt ?? false);
    }
    if (isError) {
      setStatusYt(false);
    }
  ...
  const startLivestreamMutation = useMutation({
    mutationFn: StartYouTubeLivestream,
    onSuccess: async () => {
  ...
  const stopLivestreamMutation = useMutation({
    mutationFn: StopYouTubeLivestream,
    onSuccess: async () => {
